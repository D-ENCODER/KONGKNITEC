...
class PersonalInfo(ctk.CTkFrame):
    ...
    def _personalInfoGUI(self):
        ...
        self.firstname_entry.bind('<FocusOut>', lambda event: validate_fields(parent=self.firstname_entry))
        ...
        CustomWidgets.customButton(self=self, text='BACK', command=lambda: self._controller.show_frame('Login'),
                                   ...
                                   hover_color=configure.very_dark_gray).grid(row=10, column=0, pady=10)
        CustomWidgets.customButton(self=self, text='SIGN-UP', command=None).grid(row=10, column=1, pady=10)
