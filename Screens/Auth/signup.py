...
class Signup(ctk.CTkFrame):
    ...
    def __init__(self, **kwargs):
        ...
        self.__email_error_label = ctk.CTkLabel()
        self.__password_error_label = ctk.CTkLabel()
        self.__confirm_password_error_label = ctk.CTkLabel()
    ...
    def __signupGUI(self):
        ...
        self.__email_frame = ctk.CTkFrame(master=self, fg_color=configure.very_dark_gray)
        # Calling the email entry label
        self.__email_entry = CustomWidgets.customEntry(parent=self.__email_frame, placeholder='E-mail address')
        # Placing the email entry in the grid layout
        self.__email_entry.grid(row=0, column=0, columnspan=2)
        # Placing the email frame into the grid layout
        self.__email_frame.grid(row=4, column=0, columnspan=2, pady=10)
        # Binding the validate email function to the email entry label
        self.__email_entry.bind('<FocusOut>', lambda event: validate_email(parent=self))
        ...
        self.__confirm_password_frame = ctk.CTkFrame(master=self, fg_color=configure.very_dark_gray)
        # Calling the show password button
        self.__confirm_password_entry = CustomWidgets.customEntry(parent=self.__confirm_password_frame,
                                                                  placeholder='Confirm Password', obfuscated=True)
        # Placing the confirm password entry label
        self.__confirm_password_entry.grid(row=0, column=0, columnspan=2)
        # Placing the confirm password frame into the grid layout
        self.__confirm_password_frame.grid(row=6, column=0, columnspan=2, pady=10)
        ...
        def validate_confirm_password(event=None):
            ...
            if self.__password_entry.get() == self.__confirm_password_entry.get():
                ...
                self.__confirm_password_entry.configure(border_color=configure.dark_gray)
                return True
            else:
                ...
                self.confirm_password_error_label = CustomWidgets.customErrorLabel(parent=self.__confirm_password_frame,
                ...
                self.__confirm_password_entry.configure(border_color=configure.light_cyan)
                return False

        # Binding the validate confirm password function to the confirm password entry label
        self.__confirm_password_entry.bind('<FocusOut>', validate_confirm_password)
        ...
        def show_password():
            ...
            self.__confirm_password_entry.configure(show='')
        ...
        def _verifySignup():
            ...
            if validate_email(parent=self) and validate_password(parent=self) and validate_confirm_password():
                if configure.obj.check_email_exists('Admin_details', self.__email_entry.get()) or \
                        configure.obj.check_email_exists('User_details', self.__email_entry.get()):
                    # Custom messagebox object
                    self.obj = CustomBox()
                    self.obj.errorBox('ERROR', 'Email already exists')
                else:
                    self.password = encrypt(self.__password_entry.get())
                    Signup.credentials['email'] = self.__email_entry.get()
                    Signup.credentials['password'] = self.password
                    self.__sql.insertSignupDetails(self.__email_entry.get(), self.__password_entry.get())
